        ...
        card.innerHTML = `
            ...
            <div class="card-footer">
                ...
                <div class="card-actions">
                    <a href="${item.link}" target="_blank" style="padding: 8px 16px; background: #f0f0f0; border-radius: 4px; text-decoration: none; color: #333;">Перейти</a>
                    <button class="add-to-build-btn" 
                            data-id="${item.id}" 
                            data-category="${item.category}"
                            ${inBuild || !canAdd ? 'disabled' : ''}>
                        ${inBuild ? '✓ В сборке' : 'Добавить'}
                    </button>
                </div>
            </div>
        `;
